...
File Version: 1.0.9 (May 16, 20167
Author URL: http://www.nextscripts.com
Copyright 2012-2017  NextScripts Corp
#############################################################################*/
if (!class_exists('nxs_Http')){ class nxs_Http { private $headers = '';
	function request( $url, $args = array() ) {  global $nxs_version;
		...
		if ( ( ! is_null( $r['body'] ) && '' != $r['body'] ) || 'POST' == $r['method'] || 'PUT' == $r['method'] ) { //prr($r);
	...
	function curl_request($url, $args = array()) {
		...
		curl_setopt( $handle, CURLOPT_HEADER, false ); if (stripos($r['user-agent'],'Instagram')!==false) unset($r['headers']['Content-Length']);   
		...
		$theResponse = curl_exec( $handle ); $theBody = ''; $theHeaders = nxs_Http::processHeaders( $this->headers ); //prr($theResponse, 'RESP'); prr( curl_error( $handle ), 'ERR'); prr( curl_getinfo( $handle ), 'INFO');
...
if (!function_exists("nxs_mkRemOptsArr")) {function nxs_mkRemOptsArr($hdrsArr, $ck='', $flds='', $p='', $rdr=0, $timt=45, $sslverify = false){ 
  $a = array('headers' => $hdrsArr, 'httpversion' => '1.1', 'timeout' => $timt, 'redirection' => $rdr, 'sslverify'=>$sslverify, 'User-Agent'=>'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.67 Safari/537.36'); 
