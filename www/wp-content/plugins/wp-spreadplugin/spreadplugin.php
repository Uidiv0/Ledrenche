...
if (!class_exists('WP_Spreadplugin')) {
	class WP_Spreadplugin {
		...
		public function SpreadpluginDesigner() {

			return '<div id="spreadplugin-designer-wrapper"><div id="spreadplugin-designer" class="spreadplugin-designer spreadplugin-clearfix"></div></div><script>spreadshirt.create("tablomat",{
						...
						// Refresh shopping cart
							...
							jQuery.get(ajax_object.ajaxLocation,\'action=myCart\',function (data) {
								...
								if (ajax_object.pageCheckoutUseIframe == 2) {
										var checkoutLink = jQuery(\'.spreadplugin-cart-checkout a\').attr(\'href\');
							
										if (typeof checkoutLink !== "undefined" && checkoutLink.length > 0) {
										
												jQuery(\'.spreadplugin-cart-checkout a\').magnificPopup({
										type: \'iframe\',
										callbacks: {
											close: function() {
												location.reload();
		...
		private function getSingleArticleData($pageId, $articleId, $place){
			...
			if ((int)$article['id'] > 0) {
				...
				$url = 'http://api.spreadshirt.' . self::$shopOptions['shop_source'] . '/api/v1/shops/' . self::$shopOptions['shop_id'];
				$url .= '/articles/' . $articleId . '/price?' . 'fullData=true&noCache=true';
				$objArticlePriceData = $this->runTestApiUrlWithLocaleReturnObject($url,$pageId);
				
				if ($objArticlePriceData->currency) {
					$url = (string)$objArticlePriceData->currency->attributes('http://www.w3.org/1999/xlink') . '?noCache=true';
				...
				$articleData['pricebrut'] = (float)$objArticlePriceData->vatIncluded;
		...
		private function displayArticles($id, $article, $backgroundColor = '',$isotope=true){
			...
			if (isset($article['sizes']) && is_array($article['sizes'])) {
				...
				foreach ($article['sizes'] as $k => $v) {
					$output .= '<option value="' . $k . '"'.(is_array($v) && array_key_exists('onStock',$v) && $v['onStock']==0 && self::$shopOptions['shop_stockstates'] == 1?" disabled=\"disabled\" title=\"".__('Out of stock', $this->stringTextdomain)."\"":"").'>' . (!empty($v['name']) ? $v['name'] : $k) . '</option>';
				}
			...
			if (isset($article['appearances']) && is_array($article['appearances'])) {
				...
				foreach ($article['appearances'] as $k => $v) {
					$output .= '<li value="' . $k . '"><img src="' . (!empty($v['img'])?$v['img']:$v) . '" title="'.(is_array($v) && array_key_exists('onStock',$v) && $v['onStock']==0 && self::$shopOptions['shop_stockstates'] == 1?__('Out of stock', $this->stringTextdomain):"").'" class="'.(is_array($v) && array_key_exists('onStock',$v) && $v['onStock']==0 && self::$shopOptions['shop_stockstates'] == 1?"spreadplugin-not-on-stock":"").'" /></li>';
				}
	...
	<div id="spreadplugin-tabs_content_container">
		...
		public function doRebuildCache(){
			...
			if ($action == 'getlist') {
				...
				$_mId = md5($_articleid.'.'.self::$shopOptions['shop_locale'].'.'.self::$shopOptions['shop_source'].'.'.self::$shopOptions['shop_id']);
				$_n = false;
				
				// read only when not already read, to save time and resources.
				if (!array_key_exists($_mId,$_SESSION['_tac'])) {
			...
			if (isset($_SESSION['basketUrl'][self::$shopOptions['shop_source'] . self::$shopOptions['shop_language']])) {
				...
				if (!empty($basketItems)) {
					...
					foreach ($basketItems->basketItems->basketItem as $item) {
						...
						$apiUrl = 'http://api.spreadshirt.' . self::$shopOptions['shop_source'] . '/api/v1/shops/' . (string)$item->shop['id'] . '/'.$_slug.'/' . (string)$item->element['id'];
						...
						$url = 'http://api.spreadshirt.' . self::$shopOptions['shop_source'] . '/api/v1/shops/' . (string)$item->shop['id'];
						$url .= '/'.$_slug.'/' . (string)$item->element['id'] . '/price?' . 'fullData=true&noCache=true';
						$stringXmlShop = wp_remote_get($url, array('timeout' => 120));
						if (isset($stringXmlShop->errors) && count($stringXmlShop->errors) > 0)	die('Error getting articles. Please check Shop-ID, API and secret.');
						if ($stringXmlShop['body'][0] != '<') die($stringXmlShop['body']);
						$stringXmlShop = wp_remote_retrieve_body($stringXmlShop);
						// Quickfix for Namespace changes of Spreadshirt API
						$stringXmlShop = str_replace('<ns3:', '<', $stringXmlShop);
						$objArticlePriceData = new SimpleXmlElement($stringXmlShop);
						if (!is_object($objArticlePriceData)) die('Articles prices not loaded');
						...
						<div class="cart-price"><strong>' . self::formatPrice((float)$objArticlePriceData->vatIncluded * (int)$item->quantity, '') . '</strong></div>
						</div>';
						
						$priceSum += (float)$objArticlePriceData->vatIncluded * (int)$item->quantity;
...
class SpreadpluginBasketWidget extends WP_Widget {
	...
	function widget($args, $instance){
		...
		$output = '<div class="spreadplugin-checkout widget"><span></span> <a class="spreadplugin-checkout-link' . (WP_Spreadplugin::$shopOptions['shop_basket_text_icon'] == 1 ? ' button' : '') . '">' . (WP_Spreadplugin::$shopOptions['shop_basket_text_icon'] == 0 ? __('Basket', $this->stringTextdomain) : '') . '</a></div>
