...
class nxs_Filters {
    ...
    public static function enqueue() {
        ...
        if( !empty($screen) && is_object($screen) && ($screen->id == 'nxs_filter' || $screen->id=='toplevel_page_nxssnap' || $screen->id == 'snapautoposter_page_nxssnap-accounts' || $screen->id =='toplevel_page_NextScripts_SNAP-network' || $screen->id == 'snapautoposter_page_nxssnap-settings' || $screen->id == 'snap_page_nxssnap-reposter' || $screen->id == 'snapautoposter_page_nxssnap-reposter')) { 
   ...
   <div class="nxs_tls_bdX">          
     ...
     <div id="riS<?php echo $nt; ?><?php echo $ii; ?>xd"  style="padding-top: 5px; padding-left: 0px;<?php if (isset($options['rpstType']) && $options['rpstType']=='1') echo "display:none;"; ?>"><b><?php _e('When finished', 'social-networks-auto-poster-facebook-twitter-g'); ?>:</b>       
         ...
         &nbsp;&nbsp;&nbsp;
         <input type="radio" name="nxs_rpstr[rpstStop]" value="R" <?php if (isset($options['rpstStop']) && trim($options['rpstStop'])=='R') echo 'checked="cheTcked"'; ?> /> <?php _e('Loop it. Reset and Start from the beginning', 'social-networks-auto-poster-facebook-twitter-g'); ?>
         </div>
