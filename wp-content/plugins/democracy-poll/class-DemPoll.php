...
class DemPoll {
	...
	function get_screen( $show_screen = 'vote', $before_title = '', $after_title = '' ){
		...
		$___ .= '<div id="democracy-'. $this->id .'" class="democracy" data-opts=\''. json_encode($js_opts) .'\' >';
			$___ .=  ( $before_title ?: democr()->opt('before_title') ) . democr()->kses_html( $this->poll->question ) . ( $after_title  ?: democr()->opt('after_title') );
			...
			if( democr()->cuser_can_edit_poll($this->poll) ){
				$___ .= '<a class="dem-edit-link" href="'. democr()->edit_poll_url($this->id) .'" title="'. __('Edit poll','democracy-poll') .'"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="1.5em" height="100%" viewBox="0 0 1000 1000" enable-background="new 0 0 1000 1000" xml:space="preserve"><path d="M617.8,203.4l175.8,175.8l-445,445L172.9,648.4L617.8,203.4z M927,161l-78.4-78.4c-30.3-30.3-79.5-30.3-109.9,0l-75.1,75.1 l175.8,175.8l87.6-87.6C950.5,222.4,950.5,184.5,927,161z M80.9,895.5c-3.2,14.4,9.8,27.3,24.2,23.8L301,871.8L125.3,696L80.9,895.5z"/></svg></a>';
			}

		// copyright
		if( democr()->opt('show_copyright') && ( is_home() || is_front_page() ) ){
			$___ .=  '<a class="dem-copyright" href="http://wp-kama.ru/?p=67" title="'. __('Download the Democracy Poll','democracy-poll') .'" onmouseenter="var $el = jQuery(this).find(\'span\'); $el.stop().animate({width:\'toggle\'},200); setTimeout(function(){ $el.stop().animate({width:\'toggle\'},200); }, 4000);"> © <span style="display:none;white-space:nowrap;">Kama</span></a>';
		}
		...
		$___ .= '<form method="POST" action="#democracy-'. $this->id .'">';
			$___ .= '<ul class="dem-vote">';
				...
				if( $poll->democratic && ! $this->blockVoting ){
					$___ .= '<li class="dem-add-answer"><a href="javascript:void(0);" rel="nofollow" data-dem-act="newAnswer" class="dem-link">'. _x('Add your answer','front','democracy-poll') .'</a></li>';
				}
			$___ .= "</ul>";
			...
			$___ .= '<div class="dem-bottom">';
				...
				if( ! $this->not_show_results && ! democr()->opt('dont_show_results_link') )
					$___ .= '<a href="javascript:void(0);" class="dem-link dem-results-link" data-dem-act="view" rel="nofollow">'. _x('Results','front','democracy-poll') .'</a>';
	...
	function get_result_screen(){
		...
		$___ .= '<div class="dem-bottom">';
			$___ .= '<div class="dem-poll-info">';
				$___ .= '<div class="dem-total-votes">'. sprintf( _x('Total Votes: %s','front','democracy-poll'), $total ) .'</div>';
				$___ .= ($poll->multiple  ? '<div class="dem-users-voted">'. sprintf( _x('Voters: %s','front','democracy-poll'), $poll->users_voted ) .'</div>' : '');
				$___ .= '
				...
				$___ .= $answer->added_by ? '<div class="dem-added-by-user"><span class="dem-star">*</span>'. _x(' - added by visitor','front','democracy-poll') .'</div>' : '';
				$___ .= ! $poll->open     ? '<div>'. _x('Voting is closed','front','democracy-poll') .'</div>' : '';
				if( ! $this->inArchive && democr()->opt('archive_page_id') )
					$___ .= '<a class="dem-archive-link dem-link" href="'. get_permalink( democr()->opt('archive_page_id') ) .'" rel="nofollow">'. _x('Polls Archive','front','democracy-poll') .'</a>';
			$___ .= '</div>';
	...
	function expire_time(){
		return current_time('timestamp') + ( intval( democr()->opt('cookie_days') ) * DAY_IN_SECONDS );
	}
